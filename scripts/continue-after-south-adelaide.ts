// Continue roster sync from after South Adelaide Panthers onward (finish S teams, then T-Z)
...
async function continueSync() {
  console.log('🔄 Continuing roster sync from after South Adelaide Panthers...\n');
  console.log('   (Will finish remaining S teams, then T-Z)\n');
  console.log('='.repeat(80) + '\n');
  
  // Fetch teams that come AFTER "South Adelaide Panthers" alphabetically
  const { data: teams, error } = await supabase
    .from('international_teams')
    .select('id, api_team_id, name, league_name, season_format')
    .gt('name', 'South Adelaide Panthers')
    .order('name');
  ...
  console.log(`Found ${teams.length} teams to sync (after South Adelaide Panthers)\n`);
