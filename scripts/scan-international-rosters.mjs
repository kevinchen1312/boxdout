 ...
 * 
 * Run: node scripts/scan-international-rosters.mjs
 * 
...
const EXCLUDED_LEAGUE_NAMES = [
  ...
  'gleague',
  'development league',
  // Women's leagues - various languages and formats
  'women',
  'womens',
  'woman',
  'femenina',
  ...
  'w-league',
  'ladies',
  // Youth/Junior leagues that might be mixed but we want men's pro only
  'youth',
  'junior',
  'u16',
  'u18',
  ...
  'u21',
  'u23',
  // Cup competitions (often have wrong team mappings, prefer domestic leagues)
  'super cup',
  'supercup',
  'cup',
  'trophy',
...
/**
 * Parse and normalize player names
 * API Basketball returns names in inconsistent formats:
 * - "De Colo Nando" (last first - needs fixing)
 * - "A. Atamna" (initial first - correct)
 * - "Heurtel Thomas" (last first - needs fixing)
 */
function normalizePlayerName(name) {
  if (!name) return name;
  
  // If name starts with an initial (e.g., "A. Atamna"), it's already correct
  if (/^[A-Z]\.\s/.test(name)) {
    return name;
  }
  ...
  // Detect if it's "Last First" format:
  // Heuristic: If the first word starts with uppercase and doesn't look like a first name pattern,
  // and the second word also starts with uppercase, assume it's "Last First"
  // Common patterns: Two or more parts, all capitalized
  
  // For names with 2+ parts, check if first part might be a last name
  // European last names often have: De, Van, Von, Di, Da, Le, La, Del, Della, etc.
  const prefixes = ['de', 'van', 'von', 'di', 'da', 'le', 'la', 'del', 'della', 'dos', 'das'];
  
  // Check if first word is a prefix (like "De" in "De Colo")
  ...
  // For simple two-part names, assume "Last First" and reverse
  // "Heurtel Thomas" -> "Thomas Heurtel"
  // Exception: If second part is all lowercase or looks like a suffix, don't reverse
  if (parts.length === 2) {
    const [first, second] = parts;
    // If second part is lowercase, don't reverse
    if (second === second.toLowerCase()) {
      return name;
    }
    // Reverse: assume "Last First" format
    return `${second} ${first}`;
  }
  
  // For 3+ part names without prefixes
  if (parts.length >= 3) {
    // Check if last part is a suffix (Jr., Sr., II, III, IV, etc.)
    const lastPart = parts[parts.length - 1];
    const suffixes = ['jr.', 'jr', 'sr.', 'sr', 'ii', 'iii', 'iv', 'v', '2nd', '3rd'];
    
    if (suffixes.includes(lastPart.toLowerCase())) {
      // Name already has suffix at the end, likely already in correct format
      // "LeBron James Jr." - keep as is
      return name;
    }
    
    // "Last Middle First" -> "First Last Middle"
    const firstName = parts[parts.length - 1];
    const lastName = parts.slice(0, parts.length - 1).join(' ');
    return `${firstName} ${lastName}`;
  }
...
async function getTeamRoster(teamId, teamName, leagueId, leagueName) {
  ...
  // Map to our format with normalized names
  return data.response.map(player => ({
    player_id: player.id,
    player_name: normalizePlayerName(player.name) || `Player ${player.id}`,
    team_id: teamId,
    team_name: teamName,
    league_id: leagueId,
...
async function saveMappingsBatch(mappings) {
  ...
  try {
    // Deduplicate by player_id + season (keep the first occurrence)
    const seen = new Set();
    const uniqueMappings = mappings.filter(m => {
      const key = `${m.player_id}-${m.season}`;
      if (seen.has(key)) {
        return false;
      }
      seen.add(key);
    ...
    if (uniqueMappings.length < mappings.length) {
      console.log(`  ℹ️  Deduplicated: ${mappings.length} → ${uniqueMappings.length} unique players`);
    }
    ...
    if (error) {
      ...
      console.log(`  ✅ Saved ${uniqueMappings.length} player mappings to database`);
      stats.playersFound += uniqueMappings.length;
    }
  } catch (error) {
