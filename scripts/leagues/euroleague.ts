// scripts/leagues/euroleague.ts
import { Page } from 'playwright';
import { withDebug, gotoAndWait, waitForAny, toET, retry, Row } from './common';

const LEAGUE_HOME = 'https://www.euroleaguebasketball.net/euroleague/';

const TEAM_TZ: Record<string, string> = {
  'Valencia Basket': 'Europe/Madrid',
  'ASVEL': 'Europe/Paris',
...
const REALGM_URLS: Record<string, string> = {
  'Valencia Basket': 'https://basketball.realgm.com/international/league/1/Euroleague/team/56/Valencia-Basket/schedule/2026',
  'Paris Basketball': 'https://basketball.realgm.com/international/league/1/Euroleague/team/2058/Paris-Basketball/schedule',
};
...
export async function discover(page: Page, teamLabel: string): Promise<string> {
  ...
  if (scheduleLink) {
    ...
    const directUrl = `${LEAGUE_HOME}teams/${teamSlug}/schedule/`;
...
export async function scrape(page: Page, teamLabel: string): Promise<Row[]> {
  return withDebug(page, `EuroLeague-${teamLabel}`, async () => {
